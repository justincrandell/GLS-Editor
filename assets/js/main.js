...
//* Zone Geo Json
var zoneGeoJson = JSON.parse($("#txtgeojson").val()) || {
...
document
  ...
  .addEventListener("change", function () {
    if (this.files && this.files[0]) {
      ...
      console.log("GeoJson file doesn't exist!");
    }
  });
...
function alertControl(start) {
  ...
  var cableItemList = document.getElementById("txtblink").value.split("|");
  ...
  for (let i = 0; i < alertValList.length; i++) {
    ...
    if (cableId != undefined) {
      ...
      zoneGeoJson.features.forEach((feature, featureIndex) => {
        if (
          feature.properties.text && feature.properties.text.toLowerCase().replaceAll(" ", "") ===
          sel_cable
        ) {
...
function drawCable(currentCable) {
  ...
  points && points.map((point, index) => {
    if (index === 0 || index === points.length - 1) {
      ...
      calibrationCoordinates.push(point.coordinates);
    }
  });
...
function addModifyAction(features) {
      ...
      if (isSelectCalibration) {
        ...
        layers.forEach((layer) => {
          if (
            ...
            // TODO: NEED TO MATCH CURRENT SELECTED LAYER'S TEXT
            console.log(layer.get("text"));
  ...
  modify.on("modifyend", (event) => {
    ...
    var featureText = features.getArray()[1].getProperties().text;
    zoneGeoJson.features.map((feature, featureIndex) => {
      if (feature.properties.text === featureText) {
        features
          .getArray()[1]
          .getGeometry()
          .getCoordinates()
...
function generateFeaturesInGeoJson(zoneGeoJson) {
  ...
  featureList.forEach((feature) => {
    ...
    if (featureLevel === "Feature") {
      if (feature.properties.text.indexOf("Zone") !== -1) {
        drawZone(feature);
      } else if (
...
function syncWith(event) {
  ...
  const featureText = sourceFeature.getProperties().text;
    ...
    if (map.getLayers().getLength() > 2) {
      for (let i = 2; i < map.getLayers().getLength(); i++) {
        ...
        for (const feature of features) {
          if (
            feature.getProperties().text &&
            feature.getProperties().text === featureText &&
            ...
            zoneGeoJson.features.map((zoneFeature, featureIndex) => {
              if (zoneFeature.properties.text === featureText) {
...
function onCalibrationSave() {
  ...
  zoneGeoJson.features.forEach((feature, featureIndex) => {
    if (feature.properties.text === selectedFeature.getProperties().layer) {
...
function readFile(urlPath) {
  ...
  xhr.onload = function () {
    if (xhr.status == 200) {
      ...
      if (fileContent) {
        let mapImageName = fileContent.image;
        const importedGeoJson = fileContent;

        init(importedGeoJson, mapImageName);
...
function init(
  ...
  var imageUrl = imageAssetUrl + imageName;
  ...
  function updateGeoJson() {
    ...
    zoneGeoJson.features.forEach((feature, featureIndex) => {
      var temp_key = feature.properties.text;
      if (temp_key.search("Cable") > -1) {
          ...
          features.forEach((feature) => {
            ...
            if (!layerNameList.includes(featureText)) {
              ...
              tempZoneGeoJson.features.push(
                featureText && featureText.search("Cable") > -1 ?
                generateFeatureJsonWithCalibration(
                  featureLevel,
                  featureId,
                  featureText,
                  tempCalibration[featureText],
                  featureType,
                  featureCoordinates
                ) :
                generateFeatureJson(
  ...
  $(document).on("click", "#btnSave", null, function (e) {
    ...
    // // * For Test
    // download.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(JSON.stringify({ ...zoneGeoJson, "File Name": $("#txtfnam").val() })));
    // download.setAttribute('download', "gls.txt");

    // * For Vishwa
    $.post("http://10.10.10.11", { data: JSON.stringify({ ...zoneGeoJson, "File Name": $("#txtfnam").val() }) })
      .done(function (data) {
...
map.on("pointermove", function (evt) {
  ...
  var hit = this.forEachFeatureAtPixel(evt.pixel, (feature, layer) => {
    ...
    if (
      ...
      selectedZone.features[0].getProperties().text != undefined &&
      selectedZone.features[0].getProperties().text.search("Cable") > -1
    ) {
